...
# --- EN-TÊTE AVEC LOGO ---
col_logo, col_title, col_help = st.columns([1, 3, 1])

with col_logo:
    # Remplacez l'URL par le chemin de votre fichier logo.png si vous l'avez en local
    st.image("https://picsum.photos/seed/dynia/200/80", width=100)

with col_title:
    st.title("Le Cockpit")

with col_help:
    st.markdown(f'<a href="mailto:thomas@dynia.fr" style="text-decoration:none;"><button style="border-radius:20px; border:1px solid #ccc; padding:5px 15px; cursor:pointer;">🆘 Aide</button></a>', unsafe_allow_html=True)
...
TOOLS = [
    {
        "id": "cursor",
        "name": "Cursor",
        "nickname": "Votre Espace de Création",
        "analogy": "Cursor est comme un document Word surpuissant qui vous aide à écrire des instructions pour l'ordinateur.",
        "benefit": "Il détecte les erreurs avant qu'elles n'arrivent et suggère la meilleure façon d'écrire votre code.",
        "url": "https://cursor.sh/",
        "steps": [
            "Téléchargez le fichier pour votre ordinateur (Mac ou Windows).",
            "Ouvrez le fichier et glissez Cursor dans votre dossier Applications.",
            "Ouvrez Cursor et cliquez sur 'Log In' pour connecter votre compte."
        ]
    },
    {
        "id": "warp",
    ...
    },
    {
        ...
        "steps": [
            ...
            "Connectez votre compte GitHub dans Cursor."
        ]
    }
]
...
st.markdown("""
    ...
    Avant de commencer à construire, nous devons rassembler vos outils essentiels. 
    Suivez le guide ci-dessous pour tout configurer.
""")
    ...
    with st.container():
        st.markdown(f"""
            <div class="tool-card">
                ...
                <a href="{tool['url']}" target="_blank" class="install-button">Installer {tool['name']}</a>
            </div>
        """, unsafe_allow_html=True)
        
        st.write("### 🛠️ Étapes d'installation")
...
st.markdown(f"""
    <div style="text-align: center; padding: 20px;">
        ...
        <p>Thomas est à votre disposition pour vous aider.</p>
        <a href="mailto:thomas@dynia.fr" style="background-color:#1e3799; color:white; padding:10px 20px; border-radius:10px; text-decoration:none; font-weight:bold;">Contacter Thomas</a>
    </div>
""", unsafe_allow_html=True)

if progress == 1.0:
    st.balloons()
    st.success("🎉 Félicitations ! Votre Cockpit est prêt.")